...
return function (App $app) {
    ...
    $app->post('/mahasiswa/edit', function(Request $request, Response $response) {
        ...
        // Disini dimasukan script SQL
        $sql = "INSERT INTO mahasiswa (nim, nama, jk, umur) VALUES (:nim, :nama, :jk, :umur)";
        ...
        if ($stmt->execute()) {
            return $response->withJson(["status" => "success", "data" => "Mahasiswa berhasil ditambahkan"], 200);
        } else {
            return $response->withJson(["status" => "error", "data" => "Gagal menambahkan mahasiswa"], 500);
        }
    });
